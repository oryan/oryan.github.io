...
`Some of the best [target audience] use [unconventional method or tool]`,
`I've audited/read/sold/etc. 50+ [something]'s over the past [#] months.`,
`In [#] months I'll never [something you shouldn't be able to quit] again.`,
`How I [do something people want] without spending [#]+ hours a day.`,
`[#] Steps to [get a result] on/at [situation] (without [obvious solution])`,
`Converting [something] Into [something important] (Ultimate Guide)`,
`How I [doing something time consuming] in [short amount of time]`,
`Noobs: [do something common] Pros: [do something uncommon]`,
`Here are 5 things I've learned from/doing [something uncommon]`,
`If you're new to [doing something], DON'T [non-obvious mistake]`,
`[common misconception] won't make you [rich, successful, etc]`,
`[#] Steps to [get a result] on [platform or situation] as a [niche]`,
`How to [desire] without [pain] even if you [greatest obstacle]`,
`I Stole [100M] Views/Dollars [medium] Headlines (Results)`,
`Is [doing something common] killing [the result you want]?`,
`In [date], I [did something big or new]. Since then, I [result]`,
`A microscopic amount of [topic] are leveraging [topic]`,
`I'm facing the consequences of [vulnerable weakness]`,
`Why some people almost always [get desired result].`,
`Why 99% of [target audience] Fail on/at [something]`,
`I've sent/done over [300]+ [something uncommon]`,
`The Ultimate Advice For Every [target audience]`,
`There are three key ways to [get desired result]`,
`A Day in the Life of a [job title or thing you do]`,
`10 Rules to Help You Rule The [topic] Game`,
`Do you know if you're succeeding in [topic]?`,
`My [#] biggest struggles as a [what you do]`,
`The modern evil is [something surprising]`,
`[Write] 13 [somethings] in [60] minutes`,
`Not every stage of [topic] is the same.`,
`Launch your first [topic] in 10 steps:`,
`There is no [topic] without [topic]`,
`$1 vs $1,000,000 [something]!`
]
